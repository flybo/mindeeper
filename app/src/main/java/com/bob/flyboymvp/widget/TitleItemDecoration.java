...
public class TitleItemDecoration extends RecyclerView.ItemDecoration {
    ...
    public TitleItemDecoration(Context context, List<PerAddrInfo> data) {
        super();
        mData = data;
        mPaint = new Paint();
        mBounds = new Rect();
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (position > -1) {
                if (position == 0) {//等于0的时候绘制title
                    ...
                    if (null != mData.get(position).getPer_pinyin() && !mData.get(position)
                            .getPer_pinyin().equals(mData.get(position - 1).getPer_pinyin())) {
    ...
    private void drawTitle(Canvas c, int left, int right, View child, RecyclerView.LayoutParams params, int position) {
        ...
        mPaint.getTextBounds(mData.get(position).getPer_pinyin(), 0, mData.get(position).getPer_pinyin().length(), mBounds);
        c.drawText(mData.get(position).getPer_pinyin(),
    ...
    public void getItemOffsets(Rect outRect, View view, RecyclerView parent, RecyclerView.State state) {
        ...
        if (position > -1) {
            ...
            if (position == 0) {
                ...
                if (null != mData.get(position).getPer_pinyin() &&
                        !mData.get(position).getPer_pinyin().equals(mData.get(position - 1).getPer_pinyin())) {
