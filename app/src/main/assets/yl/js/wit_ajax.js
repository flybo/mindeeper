...
//慧脑币
function ques_wmoney() {//查询慧脑币余额
	var map=new Map();map.put("token",_u.token);map.put("userguid",_u.user_guid);
	WITAJAX.ajaxmap(map,"http://121.43.233.185/mavenwitlinkweb/trade/retrievemoney.do",get_wmoney,error);
}
function get_wmoney(d){//获取慧脑币余额
	_u.money=document.getElementById("money").innerHTML=d.message.money_w||0;
}
